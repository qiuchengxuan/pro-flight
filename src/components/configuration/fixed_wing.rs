...
use crate::config::peripherals::pwm::{ServoType, PWM};
...
impl<PWMS: PwmByIdentifier, S: StaticData<ControlInput>> OnEvent for FixedWing<S, PWMS> {
    fn on_event(&mut self) {
        ...
        let config = &config::get().peripherals.pwms.0;
        ...
        for (&identifier, output) in config.iter() {
            self.pwms.with(identifier, |pwm| {
                let max_duty = pwm.get_max_duty();
                let duty = match output {
                    PWM::Motor(_) => to_motor_pwm_duty(max_duty, output.rate(), input.throttle),
                    PWM::Servo(servo) => {
