...
impl<T: Copy, const N: usize> fmt::Debug for Vec<T, N>
where
    T: fmt::Debug,
{
...
impl<T: Copy, const N: usize> Drop for IntoIter<T, N> {
    fn drop(&mut self) {
        unsafe {
...
impl<A, B: Copy, const N1: usize, const N2: usize> PartialEq<Vec<B, N2>> for Vec<A, N1>
where
    A: Copy + PartialEq<B>,
{
...
// Vec<A, N> == [B]
impl<A: Copy, B, const N: usize> PartialEq<[B]> for Vec<A, N>
where
    A: PartialEq<B>,
{
...
// Vec<A, N> == &mut [B]
impl<A: Copy, B, const N: usize> PartialEq<&mut [B]> for Vec<A, N>
where
    A: PartialEq<B>,
{
...
impl<A: Copy, B, const N: usize, const M: usize> PartialEq<[B; M]> for Vec<A, N>
where
    A: PartialEq<B>,
{
...
impl<T: Copy, const N: usize> Eq for Vec<T, N> where T: Eq {}

impl<T: Copy, const N1: usize, const N2: usize> PartialOrd<Vec<T, N2>> for Vec<T, N1>
where
    T: PartialOrd,
{
...
impl<T: Copy, const N: usize> Ord for Vec<T, N>
where
    T: Ord,
{
    #[inline]
...
impl<T: Copy, const N: usize> ops::Deref for Vec<T, N> {
    type Target = [T];
...
impl<T: Copy, const N: usize> ops::DerefMut for Vec<T, N> {
    fn deref_mut(&mut self) -> &mut [T] {
        self.as_mut_slice()
    }
}

impl<T: Copy, const N: usize> AsRef<Vec<T, N>> for Vec<T, N> {
    #[inline]
    fn as_ref(&self) -> &Self {
        self
    }
...
impl<T: Copy, const N: usize> AsMut<Vec<T, N>> for Vec<T, N> {
    #[inline]
...
impl<T: Copy, const N: usize> AsRef<[T]> for Vec<T, N> {
    #[inline]
    fn as_ref(&self) -> &[T] {
        self
    }
...
impl<T: Copy, const N: usize> AsMut<[T]> for Vec<T, N> {
    #[inline]
