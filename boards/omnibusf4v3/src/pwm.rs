...
use rs_flight::config::PwmConfigs;
...
pub fn init(pwms: PWMs, pins: PINs, clocks: Clocks, cfg: &PwmConfigs) -> ServoMixer<'static> {
    let rate1 = cfg.get(b"PWM1").map(|pwm| pwm.rate()).unwrap_or(400) as u32;
    let rate2 = cfg.get(b"PWM2").map(|pwm| pwm.rate()).unwrap_or(400) as u32;
    ...
    let rate3 = cfg.get(b"PWM3").map(|pwm| pwm.rate()).unwrap_or(50) as u32;
    let rate4 = cfg.get(b"PWM4").map(|pwm| pwm.rate()).unwrap_or(50) as u32;
    ...
    let rate = cfg.get(b"PWM5").map(|pwm| pwm.rate()).unwrap_or(50) as u32;
    let pwm5 = pwm::tim5(tim5, pa1.into_alternate_af2(), clocks, rate.hz());
    let rate = cfg.get(b"PWM6").map(|pwm| pwm.rate()).unwrap_or(50) as u32;
